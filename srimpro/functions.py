...
# plotting additional data), output = indicator for what to output (either 'plot' or 'excel', defaults to 'plot').
...
def recoilSpectra(paths, labels, n=100, num_ions=None, ion_start=0, minE=None, maxE=None, graph_style='line', 
                  xscale='log', yscale='linear', spectra='cumulative', full_range=False, excelpath=None, output='plot'):
    # docstring
    """
    ...
    full_range : bool, default=False
        option for creating secondary plot with full energy range
        (defaults to not creating this plot)
        ...
        if full_range:
            ...
            for i in range(len(paths)):
                ...
                for j in range(n):
                    ...
                    # for the first energy bin, manually sum the number of recoils at the minimum energy since these 
                    # recoils are skipped by the logic above (for the total spectra option)
                    if j == 0 and spectra == 'total':
    ...
    # if excel files given within paths, read them
        ...
        # read energy data from first excel file (assumed that all files use the same energy bins)
        energy = pd.read_excel(paths[0], usecols=[0], skiprows=0, header=None).squeeze()
        energy = pd.to_numeric(energy, errors='coerce')
        energy = np.array(energy.dropna())
        
        # add 1 more element to energy to match plotting syntax
        energy = np.append(energy, [0])
        
        # empty array to store number of recoils with each energy for each path
        numRec = np.zeros((len(paths), len(energy)-1))
    ...
    # export data to excel if excel option selected
        ...
        for i in range(len(paths)):
            ...
            data[:,0] = energy[:-1]
            data[:,1] = numRec[i]
            df = pd.DataFrame(data, columns=["PKA energy lower bound (eV)", f"{spectra} recoil fraction"])
...
def weightedRecoilSpectra(paths, labels, n=100, num_ions=None, ion_start=0, minE=None, maxE=None, graph_style='line', 
    ...
    # if excel files given within paths, read them
        ...
        # empty array to store number of recoils with each energy for each path
        weightedSpectra = np.zeros((len(paths), len(energy)-1))
    ...
    # export data to excel if excel option selected
        ...
        for i in range(len(paths)):
            ...
            df = pd.DataFrame(data, columns=["PKA energy lower bound (eV)", f"{spectra} weighted recoil fraction"])
